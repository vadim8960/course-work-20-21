...
def check_collision(screen, pixel_arr, p_begin, p_end, delta):
    if (pixel_arr[(p_begin[0] + p_end[0]) // 2, (p_begin[1] + p_end[1]) // 2] != screen.get_surface().map_rgb(
            (0, 0, 0))):
        return 0
    flag_vertic = 0
...
def main():
    ...
    points = generate_data_points(screen, pixel_arr, 1000, delta_rad, colorA, colorAaug, start_point, middle_point,
                                  end_point, size)
